...
for intft, vlant in trunk.items():     
    print('interface FastEthernet' + intft)
    for commandt in trunk_template:       
        if commandt.endswith('allowed vlan') and vlant[0] == 'add':
            print(' {} add {}'.format(commandt, ','.join(vlant[1:])))
        elif commandt.endswith('allowed vlan') and vlant[0] == 'del':
            print(' {} remove {}'.format(commandt, ','.join(vlant[1:])))
        elif commandt.endswith('allowed vlan') and vlant[0] == 'only':
            print(' {} {}'.format(commandt, ','.join(vlant[1:])))
        else:
            print(' {}'.format(commandt))  
